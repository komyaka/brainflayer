...
# Ethernet-режим (Keccak), 4 потока:
...
> обрабатывает приблизительно `N / THREADS` ключей. Итоговое число обработанных ключей
> составляет `ceil(N / THREADS) × THREADS`, что может быть слегка больше N (не более
> чем на `THREADS - 1` ключей).
